...
METHODS = {
    "anschuetz": ("separated", "anschuetz_public_original", "anschuetz_optimized"),
...
def estimate_memory(n, method, comparison, expanded, schur_backend="hahn", schur_dtype="float64",
                    locality=None, fixed_weight_cache="native"):
    """Conservative allocation guard, supplemented by the worker RSS limit."""
    from math import comb
    D = comb(n + 3, 3)
    estimate = 512 * D
    if comparison == "chang" and fixed_weight_cache == "orbit-images" and locality is not None:
        slots = sum(sum(min(side, row + locality + 1) - max(0, row - locality)
                        for row in range(side)) for side in range(n + 1, 0, -2))
        estimate += 32 * slots * comb(locality + 2, 2)
    if method.startswith("anschuetz"):
        columns = (1 if comparison == "chang" and fixed_weight_cache == "orbit-images" else
                   comb(locality + 2, 2)
                   if comparison == "chang" and method == "anschuetz_optimized" and locality is not None
                   else D)
        estimate += 3 * 16 * D * columns
        estimate += 3 * 16 * (n // 2 + 1) * (n + 1) ** 2
    if method == "separated":
        estimate += 24 * sum((m + 1) ** 2 for m in range(n + 1))
        if comparison in ("anschuetz", "chang"):
            if schur_backend == "factorial":
                # Real shared factors plus complex per-row matrix workspaces.
                estimate += (48 if schur_dtype == "float64" else 96) * 6 * (n + 1) ** 2
            else:
                estimate += (24 if schur_dtype == "float64" else 48) * sum(
                    (min(r, n - s) + 1) ** 2
...
def worker(job_path):
    ...
    import numpy as np
    from . import anschuetz_optimized, anschuetz_public
    from . import chang, georges, schur_full_ed
    from . import fixed_weight_cache, schur_separated, separated, spencer
    ...
    use_orbit_images = (job["comparison"] == "chang" and
                        cfg.get("fixed_weight_cache", "native") == "orbit-images")
    preparation_details = {}
    ...
    public = (anschuetz_public.load(cfg["anschuetz_source"])
              if method in ("anschuetz_public", "anschuetz_public_original") else None)
    if method == "anschuetz_public_original" and job["cache_mode"] != "cold":
        raise ValueError("the original public routine has no cached-application API")

    def prepare_tables():
        if use_orbit_images:
            started = time.perf_counter()
            if method == "separated":
                source_tables = schur_separated.prepare(
                    n, backend=cfg.get("schur_backend", "hahn"),
                    dtype=np.dtype(cfg.get("schur_dtype", "float64")))
            elif method == "chang":
                source_tables = chang.prepare(n, job["locality"])
            else:
                source_tables = None
            ...
            def build_orbit(key):
                nonlocal image_seconds
                start = time.perf_counter()
                if method == "separated":
                    blocks = schur_separated.pauli_to_schur(n, {key: 1.0}, source_tables)
                elif method == "chang":
                    blocks = chang.pauli_to_schur(n, {key: 1.0}, source_tables)
                elif method == "anschuetz_optimized":
                    blocks = anschuetz_optimized.f_blocks_for_key(n, key)
                else:
                    raise ValueError("unsupported orbit-image construction method")
                image_seconds += time.perf_counter() - start
                return blocks

            bank = fixed_weight_cache.prepare(n, job["locality"], build_orbit)
            preparation_details.update(
                source_table_seconds=source_seconds,
                orbit_construction_seconds=image_seconds,
                cache_packing_seconds=max(0.0, time.perf_counter() - started - source_seconds - image_seconds))
            return bank
        if method == "separated":
            return (schur_separated.prepare(n, backend=cfg.get("schur_backend", "hahn"),
                                            dtype=np.dtype(cfg.get("schur_dtype", "float64")))
                    if schur_task else separated.prepare(n))
        if method == "anschuetz_public":
            return anschuetz_public.prepare(n, public)
        if method == "anschuetz_optimized":
            # Fixed-weight inputs need only their active orbit columns. These
            # depend on n and the support, not on the sampled amplitudes.
            return anschuetz_optimized.prepare(n, keys=coefficients if job["comparison"] == "chang" else None)
        if method == "chang":
            return chang.prepare(n, job["locality"])
        return None

    reusable = method in ("separated", "anschuetz_public", "anschuetz_optimized", "chang")
    ...
    def one_sample(repeat, warmup=False):
        ...
        row.update(repeat=repeat, algorithm_seed=job["input_seed"] + 97531 * (repeat + 1),
                   ...
                   backend={"anschuetz_public_original": "pinned_public_construct_matrix_blocks",
                            "anschuetz_public": "pinned_public_F_columns",
                            "anschuetz_optimized": "constrained_Appendix_E_DP",
                            "separated": ("numpy_Krawtchouk_factorial" if cfg.get("schur_backend", "hahn") == "factorial"
                                          else "numpy_Krawtchouk_Hahn") if schur_task else "numpy_Krawtchouk",
                            "chang": "Algorithm_1_corrected_normalization_CSR",
                            "georges": "numpy_FWHT", "spencer": "pinned_sparse_row_oracle"}[method])
        if method == "separated" and schur_task:
            row["schur_dtype"] = cfg.get("schur_dtype", "float64")
        if method == "anschuetz_optimized":
            row["anschuetz_table_scope"] = ("active_fixed_weight_orbits" if job["comparison"] == "chang"
                                            else "complete_orbit_bank")
        if job["comparison"] == "chang":
            row["fixed_weight_cache"] = "orbit-images" if use_orbit_images else "native"
        oracle = None
        try:
            phase("setup")
            ...
            if use_orbit_images:
                row.update(preparation_details)
                row["orbit_image_count"] = len(tables.keys)
                row["cache_band_slots"] = tables.values.shape[0]
                row["max_forbidden_abs"] = tables.max_forbidden_abs
                row["max_forbidden_relative"] = tables.max_forbidden_relative
            phase("input_preparation")
            if use_orbit_images:
                packed, row["preparation_seconds"] = timed(
                    lambda: fixed_weight_cache.pack_coefficients(tables, coefficients))
            if method == "georges":
            ...
            def apply():
                if use_orbit_images:
                    return fixed_weight_cache.combine(tables, packed)
                if method == "separated":
                ...
                if method == "anschuetz_public_original":
                    # Invoke the author's complete routine on every sample. No
                    # adapter-built F cache or replacement accumulation is used.
                    return anschuetz_public.pauli_to_schur(n, coefficients, reference=public)
                if method == "anschuetz_optimized":
            ...
            def format_output():
                if use_orbit_images:
                    return fixed_weight_cache.format_blocks(tables, raw)
                if job["comparison"] == "chang" and method != "chang":
            ...
            started = time.perf_counter()
            if schur_task:
                metrics = validate_schur(n, formatted if use_orbit_images else raw,
                                         coefficients, reference, atol, rtol)
                if job["comparison"] == "chang":
...
def parser():
    ...
    p.add_argument("--schur-backend", choices=("hahn", "factorial"), default="hahn",
                   help="Schur conversion implementation; factorial implements Chapter 5")
    p.add_argument("--schur-dtype", choices=("float64", "longdouble"), default="float64",
                   help="Schur-stage precision; Krawtchouk tables and outputs remain double precision")
    ...
    p.add_argument("--anschuetz-implementations", nargs="+",
                   choices=("public_original", "public", "optimized"),
                   default=["public_original", "optimized"],
                   help="public_original calls the original complete routine (cold only); "
                        "public retains the historical adapter-cached F comparison")
    p.add_argument("--anschuetz-public-max-n", type=int,
                   help="explicit size cap for public methods; larger cases are recorded as size-limit skips")
    p.add_argument("--fixed-locality-anschuetz", action="store_true",
                   help="add optimized Anschuetz to the Chang comparison, preparing only active orbit columns")
    p.add_argument("--fixed-weight-cache", choices=("native", "orbit-images"), default="native",
                   help="fixed-weight preparation: native tables or a common sparse bank of all orbit images")
...
def main(argv=None):
    ...
    if args.anschuetz_public_max_n is not None and args.anschuetz_public_max_n < 1:
        p.error("anschuetz-public-max-n must be positive")
    if args.fixed_locality_anschuetz and args.comparison != "chang":
        p.error("--fixed-locality-anschuetz requires --comparison chang")
    if args.fixed_weight_cache != "native" and args.comparison != "chang":
        p.error("--fixed-weight-cache orbit-images requires --comparison chang")
    ...
    if args.fixed_locality_anschuetz:
        methods.append("anschuetz_optimized")
    ...
    cfg["method_cache_modes"] = {
        method: [mode for mode in modes if method != "anschuetz_public_original" or mode == "cold"]
    ...
    for n in n_values:
        for locality in (localities if family != "general" else [0]):
            for instance in range(args.instances):
                ...
                jobs = [(method, mode) for method in methods for mode in cfg["method_cache_modes"][method]]
                order.shuffle(jobs)
                for method, mode in jobs:
                    ...
                    problem = guarded
                    if not problem and estimate_memory(n, method, args.comparison, args.output_contract == "expanded",
                                                       args.schur_backend, args.schur_dtype,
                                                       locality=locality,
                                                       fixed_weight_cache=args.fixed_weight_cache) > args.max_memory_mb * 1024 ** 2:
                        problem = "allocation_guard"
                    if args.comparison == "spencer" and support > args.max_pauli_terms:
                        problem = "pauli_support_guard"
                    if method in ("anschuetz_public", "anschuetz_public_original"):
                        if not public_path.is_file():
                            problem = "missing_public_source"
                        elif args.anschuetz_public_max_n is not None and n > args.anschuetz_public_max_n:
                            problem = "configured_size_limit"
