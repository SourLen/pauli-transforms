...
from . import dense_reference, transforms
...
def make_inputs(n, seed):
    ...
    coefficients = transforms.schur_to_pauli(n, state, backend="hahn")
...
def independent_reference(n, inputs, generated_state, *, dense_max_n=5, tolerance=1e-8):
    ...
    dense = None
    if n <= dense_max_n:
        dense = tuple(dense_reference.dense_operator(n, mapping) for mapping in inputs)
