"""Coordinate conventions and small-system conversions.

A Pauli key ``(w,g0,g1)`` means counts
``(nI,nX,nY,nZ) = (n-w-g0,w-g1,g1,g0)``. Its value is the common
coefficient of each individual Pauli word, equivalently the coefficient
multiplying the unnormalized orbit sum.

An entry key ``(w,h0,h1)`` stores the literal matrix entry for a row/column
pair with w differing bits, h0 common ones, and h1 column ones among the
differing bits. In the thesis's ``(r,s,t)`` notation this is
